...
print("--- Parte con le spline---")

f2.1 = as.formula(paste("OutAdoption ~ s(", paste(names(train)[-c(1)], collapse=")+s("),")", collapse=NULL)) # Uso i nomi di train così esclude le nuove colonne
f2.2 = as.formula(paste("OutDied ~ s(", paste(names(train)[-c(1)], collapse=")+s("),")", collapse=NULL))
f2.3 = as.formula(paste("OutEuthanasia ~ s(", paste(names(train)[-c(1)], collapse=")+s("),")", collapse=NULL))
f2.4 = as.formula(paste("OutReturnToOwner ~ s(", paste(names(train)[-c(1)], collapse=")+s("),")", collapse=NULL))
f2.5 = as.formula(paste("OutTransfer ~ s(", paste(names(train)[-c(1)], collapse=")+s("),")", collapse=NULL))
...
# trainAlt contiene già l'rbind di train e test
print("Calcolo modelli comparazione...")
mgam.model.comparsion.s.1 = gam(f2.1, data=trainAlt, family=binomial)
mgam.model.comparsion.s.2 = gam(f2.2, data=trainAlt, family=binomial)
mgam.model.comparsion.s.3 = gam(f2.3, data=trainAlt, family=binomial)
mgam.model.comparsion.s.4 = gam(f2.4, data=trainAlt, family=binomial)
mgam.model.comparsion.s.5 = gam(f2.5, data=trainAlt, family=binomial)

print("Calcolo modelli completi...")
mgam.model.full.s.1 = gam(f2.1, data=datasetAlt, family=binomial)
mgam.model.full.s.2 = gam(f2.2, data=datasetAlt, family=binomial)
mgam.model.full.s.3 = gam(f2.3, data=datasetAlt, family=binomial)
mgam.model.full.s.4 = gam(f2.4, data=datasetAlt, family=binomial)
mgam.model.full.s.5 = gam(f2.5, data=datasetAlt, family=binomial)

# Predizione sul test di validazione, prima normalizzo le probabilità
print("Predizione sui modelli di comparazione...")
mgam.predictions.comparsion.s.probs.1 = predict(mgam.model.comparsion.s.1, validation, type="response") # Probabilità del livello yes
mgam.predictions.comparsion.s.probs.2 = predict(mgam.model.comparsion.s.2, validation, type="response")
mgam.predictions.comparsion.s.probs.3 = predict(mgam.model.comparsion.s.3, validation, type="response")
mgam.predictions.comparsion.s.probs.4 = predict(mgam.model.comparsion.s.4, validation, type="response")
mgam.predictions.comparsion.s.probs.5 = predict(mgam.model.comparsion.s.5, validation, type="response")
.s
print("Aggiusto le probabilità e le classi...")
mgam.predictions.comparsion.s.probs = matrix(nrow=length(mgam.predictions.comparsion.s.probs.1), ncol=5)
mgam.predictions.comparsion.s.class = rep(0,length(mgam.predictions.comparsion.s.probs.1))
for (i in 1:length(mgam.predictions.comparsion.probs.1)) {
    v = c(mgam.predictions.comparsion.s.probs.1[i],
          mgam.predictions.comparsion.s.probs.2[i],
          mgam.predictions.comparsion.s.probs.3[i],
          mgam.predictions.comparsion.s.probs.4[i],
          mgam.predictions.comparsion.s.probs.5[i])
    v = v/sum(v) # Normalizza le probabilità
    mgam.predictions.comparsion.s.probs[i,] = v
    class = which.max(v)
    fclass = ""
    if (class == 1){ fclass = "Adoption"}
    else if (class== 2) {fclass = "Died"}
    ...
    mgam.predictions.comparsion.s.class[i] = fclass
}
mgam.predictions.comparsion.s.class = as.factor(mgam.predictions.comparsion.s.class)

# Predizione delle probabilità per la sfida di kaggle (sul dataset separato)
print("Predizione sul modello completo...")
mgam.predictions.full.s.probs.1 = predict(mgam.model.full.s.1, kaggle.dataset, type="response") # Probabilità del livello yes
mgam.predictions.full.s.probs.2 = predict(mgam.model.full.s.2, kaggle.dataset, type="response")
mgam.predictions.full.s.probs.3 = predict(mgam.model.full.s.3, kaggle.dataset, type="response")
mgam.predictions.full.s.probs.4 = predict(mgam.model.full.s.4, kaggle.dataset, type="response")
mgam.predictions.full.s.probs.5 = predict(mgam.model.full.s.5, kaggle.dataset, type="response")
print("Aggiusto le probabilità...")
mgam.predictions.full.s.probs = matrix(nrow=length(mgam.predictions.full.s.probs.1), ncol=5)
for (i in 1:length(mgam.predictions.full.s.probs.1)) {
    v = c(mgam.predictions.full.s.probs.1[i],
          mgam.predictions.full.s.probs.2[i],
          mgam.predictions.full.s.probs.3[i],
          mgam.predictions.full.s.probs.4[i],
          mgam.predictions.full.s.probs.5[i])
    v = v/sum(v) # Normalizza le probabilità
    mgam.predictions.full.s.probs[i,] = v
}
...
mgam.misc.error = 1 - sum(diag(mgam.misc.table))/sum(mgam.misc.table)
print("Scrivo le predizioni...")
writePredictions(mgam.predictions.full.s.probs, kaggle.dataset[,1], "previsioni/mgam.csv")